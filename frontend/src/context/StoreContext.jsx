...
const StoreContextProvider = (props) => {
  const [cartItems, setCartItems] = useState({}); // état local pour stocker les éléments du panier, initialisé à un objet vide
  ...
  const addToCart = (itemId) => {
    if (!cartItems[itemId]) {
      setCartItems((prev) => ({ ...prev, [itemId]: 1 })); // si l'item n'est pas encore dans le panier, on l'ajoute avec une quantité de 1 et les .... nous sert à conserver les éléments précédents du panier tout en ajoutant le nouvel item avec sa quantité car setCarditems remplace l'état précédent par le nouvel état, en utilisant la syntaxe de mise à jour fonctionnelle de useState, on peut accéder à l'état précédent (prev) et le mettre à jour en ajoutant le nouvel item avec sa quantité. Cela garantit que les éléments précédents du panier ne sont pas perdus lors de l'ajout d'un nouvel item.
    } else {
      setCartItems((prev) => ({ ...prev, [itemId]: prev[itemId] + 1 })); // si l'item est déjà dans le panier, on augmente sa quantité de 1
    }
  }
  ...
  const removeFromCart = (itemId) => {
    setCartItems((prev) => ({ ...prev, [itemId]: prev[itemId] - 1 })); // si l'item est dans le panier, on diminue sa quantité de 1
  }

useEffect(() => {
  console.log(cartItems); // Affiche le contenu du panier dans la console à chaque fois qu'il est mis à jour, ce qui permet de suivre les changements dans le panier en temps réel.
}, [cartItems]);
